...
import { DEFAULT_PROBABILITIES, RARITIES, drawOne, validateProbabilities } from './core.js';
...
const emptyDb = () => ({ settings: { probabilities: { ...DEFAULT_PROBABILITIES }, pityRules: [] }, participants: [], records: [], eventSeq: 0, lastEvent: null });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && parts[1] === 'api' && parts[2] === 'participants' && parts[4] === 'draw') {
      ...
      for (let i = 0; i < count; i++) { const result = drawOne({ probabilities: db.settings.probabilities, pityRules: db.settings.pityRules, pity, mode, forcedRarity: i === 0 ? input.forcedRarity : undefined }); pity = result.nextPity; results.push({ ...result, index: i }); }
      ...
      db.eventSeq += 1; db.lastEvent = { type: 'draw', participant: participant(p), results, batchId }; save(); return sendJson(res, 200, db.lastEvent);
    }
