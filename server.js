...
const emptyDb = () => ({ settings: { probabilities: { ...DEFAULT_PROBABILITIES }, pityRules: [], tenPullMode: 'random', constraintEnabled: true, maxLow: 2 }, participants: [], records: [], eventSeq: 0, lastEvent: null });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/settings') { const input = await readBody(req); const checked = validateProbabilities(input.probabilities || {}); if (!checked.valid) return sendJson(res, 400, { error: checked.error }); db.settings = { probabilities: checked.values, pityRules: (input.pityRules || []).map((r) => ({ rarity: r.rarity, limit: Number(r.limit), enabled: Boolean(r.enabled) })).filter((r) => RARITIES.includes(r.rarity) && Number.isInteger(r.limit) && r.limit > 0), tenPullMode: input.tenPullMode === 'constrained' ? 'constrained' : 'random', constraintEnabled: input.constraintEnabled !== false, maxLow: Math.max(0, Math.min(10, Number(input.maxLow ?? 2))) }; save(); return sendJson(res, 200, { ...db.settings, usedDefault: checked.usedDefault }); }
