...
// A fresh Overlay session starts blank; participant data and settings remain persistent.
db.lastEvent = null;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/events') { if (url.searchParams.get('init') === '1') return sendJson(res, 200, { seq: db.eventSeq, event: null }); return sendJson(res, 200, db.eventSeq > Number(url.searchParams.get('after') || 0) ? { seq: db.eventSeq, event: db.lastEvent } : { seq: db.eventSeq, event: null }); }
